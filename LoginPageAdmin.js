...
function LoginAdmin(){
    ...
    const Validation=(e)=>{
        ...
        axios.get('https://localhost:44317/api/Candidate/GetCandidate',{
            headers:{
              'Authorization':`Bearer ${JSON.parse(localStorage.getItem('Authtoken'))}`
            }
          }).then((response) => {
            setCandidate(response.data);
            localStorage.setItem("TotalParticipants",candidate.length);
            console.log(localStorage.getItem("TotalParticipants"));
